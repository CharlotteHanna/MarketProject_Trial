...
def create_payment(request: PaymentsBase, db: Session = Depends(get_db), current_user: DbUser = Depends(get_current_user)):
    if db_products.get_seller_id(db, request.paid_product_id) == current_user.user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Owner of a product cant start conversation about a product he owns')
...
def update_payment(id: int, db: Session = Depends(get_db), current_user: DbUser = Depends(get_current_user)):
